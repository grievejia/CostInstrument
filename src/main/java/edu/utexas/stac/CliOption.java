...
public class CliOption {
    ...
    @CommandLine.Option(names = { "--output-java-version" }, description = "Specify the Java version of the output " +
            "file. Valid range is from 1 to 8, and 0 means the default")
    ...
    @CommandLine.Option(names = { "-o", "--output" }, description = "The output jar file (default to ./instrumented" +
            ".jar)")
    ...
    public void validate() {
        if (outputJavaVersion < 0 || outputJavaVersion > 8)
            throw new RuntimeException("Illegal output Java version: " + outputJavaVersion);
    }
}
